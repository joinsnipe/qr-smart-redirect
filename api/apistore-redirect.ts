...
/** Cuenta cuántas páginas existen ya para una campaña (archivadas no) */
async function countPagesForCampaign(notionToken: string, dbId: string, campaignRaw: string): Promise<number> {
  ...
  // Filtramos por Campaign (rich_text equals) y no archivadas
  do {
    const body = {
      page_size: 100,
      ...
      filter: {
        and: [
          { property: "Campaign", rich_text: { equals: campaignRaw } },
          { property: "archived", checkbox: { equals: false } } // este no suele estar; por si no existe lo ignorará
        ].filter(Boolean)
      }
    };
    ...
    if (!res.ok) {
      // Si el filtro de "archived" no existe en tu DB, usamos solo Campaign
      const fallbackRes = await fetch(`https://api.notion.com/v1/databases/${dbId}/query`, {
        method: "POST",
        headers,
        body: JSON.stringify({
          page_size: 100,
...
export default async function handler(req: Request) {
  ...
  // --- calcula seq por campaña (conteo + 1) ---
  let seqC = 1;
  try {
    const notionToken = process.env.NOTION_TOKEN!;
    const notionDbId  = process.env.NOTION_DB_ID!;
    const count = await countPagesForCampaign(notionToken, notionDbId, campaignRaw);
    seqC = count + 1;

    // crea la página con Name “Campaña Bonita / 00X”
    const headers = {
      "Authorization": `Bearer ${notionToken}`,
      "Notion-Version": "2022-06-28",
      "Content-Type": "application/json"
    ...
    const properties: any = {
      "Name":        { title: [{ text: { content: `${campaignPretty} / ${pad3(seqC)}${qrId ? ` / ${qrId}` : ""}` } }] },
      "SeqC":        { number: seqC },
    ...
    // si falla el conteo/insert, no rompemos UX
    // (alternativamente, podríamos crear sin seqC)
  }
